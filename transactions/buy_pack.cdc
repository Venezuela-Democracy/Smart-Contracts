...
transaction(setID: UInt32) {
    ...
    prepare(signer: auth(BorrowValue, SaveValue) &Account) {
        ...
        let receipt <- VenezuelaNFT_4.buyPack(setID: setID)
        
        // Check that I don't already have a receipt stored
        if signer.storage.type(at: VenezuelaNFT_4.ReceiptStoragePath) != nil {
            panic("Storage collision at path=".concat(VenezuelaNFT_4.ReceiptStoragePath.toString()).concat(" a Receipt is already stored!"))
        }

        // Save that receipt to my storage
        // Note: production systems would consider handling path collisions
        signer.storage.save(<-receipt, to: VenezuelaNFT_4.ReceiptStoragePath)
    }
}