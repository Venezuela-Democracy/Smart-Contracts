...
transaction(listingResourceID: UInt64, storefrontAddress: Address, commissionRecipient: Address?) {
    ...
    let VenezuelaNFT_18Receiver: &{NonFungibleToken.Receiver}
    ...
    prepare(acct: auth(BorrowValue) &Account) {
        ...
        self.VenezuelaNFT_18Receiver = acct.capabilities.borrow<&{NonFungibleToken.Receiver}>(collectionData.publicPath)
...
/*         if commissionRecipient != nil && commissionAmount != 0.0 {
    ...
    execute {
        ...
        self.VenezuelaNFT_18Receiver.deposit(token: <-item)
    }
}