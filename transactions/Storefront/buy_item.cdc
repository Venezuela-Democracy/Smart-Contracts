...
transaction(listingResourceID: UInt64, storefrontAddress: Address, commissionRecipient: Address?) {
    ...
    let VenezuelaNFT_15Receiver: &{NonFungibleToken.Receiver}
    ...
    prepare(acct: auth(BorrowValue) &Account) {
        ...
        self.VenezuelaNFT_15Receiver = acct.capabilities.borrow<&{NonFungibleToken.Receiver}>(collectionData.publicPath)
...
/*         if commissionRecipient != nil && commissionAmount != 0.0 {
    ...
    execute {
        ...
        self.VenezuelaNFT_15Receiver.deposit(token: <-item)
    }
}