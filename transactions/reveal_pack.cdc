...
/*         let receipt <- signer.storage.load<@VenezuelaNFT_18.Receipt>(from: VenezuelaNFT_18.ReceiptStoragePath)
            ?? panic("No Receipt found in storage at path=".concat(VenezuelaNFT_18.ReceiptStoragePath.toString())) */

        // Reveal by redeeming my receipt - fingers crossed!
        VenezuelaNFT_18.revealPack(receipt: <- receipt, minter: signer.address, emptyDict: {})
