...
access(all) fun main(account: Address): [AnyStruct]?  {
    ...
    let cap = account.capabilities.borrow<&VenezuelaNFT_19.Collection>(VenezuelaNFT_19.CollectionPublicPath)!
    log(cap)
    ...
    for id in ids {
        ...
        let cardType = VenezuelaNFT_19.getCardType(cardID: UInt32(id))
